...
GPUの中にもnVIDIAやAMDなどがある。  
nVIDIAのGPUを動かすためのCUDA。

GPUだと一つ一つのコアは性能が少ないが、数がめちゃ多い。  
（CPUは逆）
...
NVIDIAが開発・提供しているGPU向けの「汎用並列コンピューティングプラットフォーム」
...
- C言語ライクに書けるCUDA用言語
- CUDAで実行したいところだけ修正すれば使える！（他はCで書いている）

## pyCUDAとは？

- pythonでCUDAを書くためのライブラリ
- CUDA Cで書いた内容をそのままpyCUDAで使用可能

## GPUコンピューティング

GPUはコア数も多いし、メモリが一体化しているのでアクセスが早いというメリットもある。  
（CPUだとメモリからとってくるロスがある）


並列化しにくい時はCPU、並列化しやすい時はGPU  
→ すべてGPUで計算するわけではない
